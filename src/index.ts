...
import { runFeatureSimulation } from "./simulate.js";
...
async function main(): Promise<void> {
  ...
  try {
    switch (command) {
      ...
      case "simulate-discord": {
        await runFeatureSimulation(webhook);
        console.log("Simulation de toutes les fonctionnalites envoyee sur le webhook configure.");
        break;
      }
      case "weekly-recap": {
        const result = await tracker.postWeeklyRecap();
        console.log(result.posted ? "Recap hebdomadaire envoye." : "Aucun recap a envoyer.");
        if (result.failures.length > 0) {
          console.error(result.failures.join("\n"));
        }
        break;
